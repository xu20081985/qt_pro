...
{
    while (netconfig_pro->canReadLine())
        {
           QByteArray t=netconfig_pro->readAll();
...
void MainWindow::on_ok_clicked()
{
    ...
    if(ui->ok->text()=="stop")
    {
        ::system("kill -9 $(ps | grep udhcpc | awk '$0 !~/grep/ {printf $1}')");
        ::system("kill -9 $(ps | grep ifconfig | awk '$0 !~/grep/ {printf $1}')");
        ::system("kill -9 $(ps | grep route | awk '$0 !~/grep/ {printf $1}')");
...
//    this->setDisabled(true);
    ...
    netconfig_pro = new QProcess;

    connect(netconfig_pro, SIGNAL(readyRead()), this, SLOT(readOutput()));

    if(i->dhcp)
    {
        cm="udhcpc -i "+i->name;
//        ::system(cm.toLatin1().data());
        netconfig_pro->start(cm);
        if(!netconfig_pro->waitForStarted())
        {
            QMessageBox::warning(this,tr("warning"),tr("udhcpc command execute error!"));
            return;
        }
    }
    else
    {
        QString nulstr="...";
        if(i->ip.compare(nulstr)==0)
        {
            QMessageBox::warning(this,tr("warning"),"Please input ip");
            ui->ok->setEnabled(true);
            return;
        }else if(i->mask.compare(nulstr)==0)
        {
            QMessageBox::warning(this,tr("warning"),"Please input mask");
            ui->ok->setEnabled(true);
            return;
        }else if(i->gateway.compare(nulstr)==0)
        {
            QMessageBox::warning(this,tr("warning"),"Please input gateway");
            ui->ok->setEnabled(true);
            return;
        }
 ...
 //       ::system(cm2.toLatin1().data());
        ...
        if(!netconfig_pro->waitForStarted())
        {
            QMessageBox::warning(this,tr("warning"),tr("ifconfig command execute error!"));
            return;
        }
        netconfig_pro->start(cm2);
        if(!netconfig_pro->waitForStarted())
        {
            QMessageBox::warning(this,tr("warning"),tr("toute command execute error!"));
            return;
        }
        ui->textEdit->setText("set static ip finished");
    }
