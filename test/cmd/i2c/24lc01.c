                                               ...
                                               __u8 length, __u8 *values)
{
	...
	for (i = 1; i <= length; i++)
	{
                  ...
                  // fprintf(stderr,"values=%d",values[i-1]);
        }
	data.block[0] = length;
	return i2c_smbus_access(file,I2C_SMBUS_WRITE,command,
...
static int i2c_write_2b(struct eeprom *e, __u8 buf[2])
{
	int r;
	// we must simulate a plain I2C byte write with SMBus functions
        //fprintf(stderr, "write buf[0]=: %d\n", buf[0]);
        // fprintf(stderr, "write buf[1]=: %d\n", buf[1]);
        //fprintf(stderr, "write e->fd=: %d\n", e->fd);
...
int eeprom_open(char *dev_fq, int addr, int type, struct eeprom* e)
{
	int funcs, fd, r;
	e->fd = e->addr = 0;
	e->dev = 0;
	
	fd = open(dev_fq, O_RDWR);
	if(fd <= 0)
	{
...
int eeprom_read_byte(struct eeprom* e, __u16 mem_addr)
{
	...
	r = i2c_smbus_read_byte(e->fd);
       ...
       //fprintf(stderr, "read e->addr=: %d\n", e->addr);
       // fprintf(stderr, "read e->atype=: %d\n", e->type);
       // fprintf(stderr, "read e->devr=: %s\n", e->dev);
...
int write_to_block(struct eeprom *e)
{
        __u8 *str="abcdefg";
        int  length=7;
	
        return i2c_smbus_write_block_data(e->fd,0,length, str);
}
